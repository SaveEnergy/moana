...
// transactions, aggregates (aggregate_sum.go including SumIncomeExpenseCentsInRange, SumIncomeExpenseCentsInTwoRanges, SumRunningTotalAndIncomeExpenseInTwoRanges, aggregate_category.go including ListCategoryAmountsInRange + ErrInvalidCategoryAmountKind, aggregate_time.go sqlFilterOccurredAtFrom/To + appendOccurredAtRange; household SQL fragments in transaction_sql.go; income/expense AND filters in sql_amount_filter.go), and
// daily movement for the heatmap; per-user notifications (notification_*.go including [Store.MarkNotificationRead] and [Store.CountUnreadNotificationsForUser]; migrations v9–v10). Open via [moana/internal/dbutil.OpenStore].
...
// SQLite TEXT timestamps use moana/internal/timeutil (ParseSQLiteTimestamp, ParseSQLiteTimestampUTC, FormatSQLiteUTC, NowSQLiteUTC).
