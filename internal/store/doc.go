...
// User types in user_types.go; scanning in user_scan.go; reads in user_get.go, user_list.go (preallocated slices), user_sql.go; profile/password/create in user_mutate.go ([CreateUser] returns [ErrDuplicateUserEmail] on unique email).
