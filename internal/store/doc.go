...
// Household type in household.go; reads in household_query.go (+ household_sql.go); writes in household_mutate.go (name, invites, detach; [ErrDuplicateUserEmail] from member insert).
// User types in user_types.go; scanning in user_scan.go; reads in user_get.go, user_list.go (preallocated slices), user_sql.go (full-row get-by-email/get-by-id + admin/household member lists); profile/password/create in user_mutate.go ([CreateUser] trims email, returns [ErrDuplicateUserEmail] or [ErrInvalidUserEmail]).
