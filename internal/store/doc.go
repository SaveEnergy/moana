...
// User types in user_types.go; scanning in user_scan.go; reads in user_get.go, user_list.go (preallocated slices), user_sql.go (full-row get-by-email/get-by-id); profile/password/create in user_mutate.go ([CreateUser] trims email, returns [ErrDuplicateUserEmail] or [ErrInvalidUserEmail]).
