...
// Transaction types in transaction_types.go; reads in transaction_get.go, transaction_list.go (+ transaction_sql.go list prefix, sql/row helpers, LIKE escape in like_escape.go); writes in transaction_mutate.go (create/update + category validation helpers).
