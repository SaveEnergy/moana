...
func registerSettingsRoutes(mux *http.ServeMux, app *App) {
	...
	mux.Handle(http.MethodPost+" "+SettingsAvatarPath, app.WithAuth(app.SettingsAvatarUpload))
	mux.Handle(http.MethodPost+" "+SettingsAvatarRemovePath, app.WithAuth(app.SettingsAvatarRemove))
	mux.Handle(http.MethodGet+" "+AvatarsPathPattern, app.WithAuth(app.UserAvatarGet))
