...
// routes_settings.go, routes_notifications.go (GET /notifications [App.Notifications], POST /notifications/read [App.NotificationMarkRead], notifications.html). Form field names for login/settings: formnames.go. Auth/session: [App.CurrentUser], [App.WithAuth]. Rendering: layout.go,
