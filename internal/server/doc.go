// Package server wires the HTTP mux: static assets with cache headers (static.go, static_test.go), health (health.go), request logging (logging.go, logging_test.go; GET [HealthPath] skips logging so probes stay cheap),
