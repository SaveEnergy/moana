...
- **Settings:** Float-field forms, readonly **Email** field `#settings-email` bound to the signed-in user, add-member (**`domSelectors.ts`**: **`SETTINGS_ADD_MEMBER_*`**, **`SETTINGS_ADD_MEMBER_DIALOG_SELECTOR`**, **`SETTINGS_ADD_MEMBER_OPEN_SELECTOR`**; **`initSettingsMemberDialog`** uses **`queryBootContent`** (**`SETTINGS_ADD_MEMBER_DIALOG_SELECTOR`**, **`contentRoot.ts`**); **`querySettingsAddMemberOpenButton`** on **`dialog.parentElement`** when attached (else **`resolveBootContentQueryRoot()`** — same node **`queryBootContent`** used for the dialog) in **`settingsMemberDialog.ts`**; **`attachNativeDialogDismiss`**: backdrop, close, Cancel), member list with role badges  
