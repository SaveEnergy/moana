...
- **Confirm-before POST:** `form[data-confirm="…"]` — `window.confirm` on **`submit`** (`confirmSubmitForms.ts`); avoid inline handlers
...
- **Unit:** `npm run test:unit` — Vitest, config **merged into `frontend/vite.config.ts`** (`test` block). Covers `frontend/src/lib/*`, **`bootApp()`** / **`BOOT_APP_INITIALIZERS`** wiring and initializer order (`boot.test.ts`), timezone cookie segment + **`parseMoanaTimezoneCookie`**, **`shouldDeferMobileShellEscape`** and related helpers (`dialogKeyboard.test.ts`), `formatLocalTimeLabel` / `applyLocalTimeElements`, category custom hex sanitization (`categoryColor.test.ts`), **`onMediaQueryChange`** (`shellBreakpoints.test.ts`), **`clickEventTargetElement`** (`clickTarget.test.ts`), **`shouldCloseNativeDialogFromClick`** (`dialogDismiss.test.ts`), **`shouldCloseMobileSidebarFromShellClick`** (`mobileShellDismiss.test.ts`), **`wireHistorySortAutoSubmit`** (`historyControls.test.ts`), **`attachConfirmBeforeSubmit`** (`confirmSubmitForms.test.ts`).  
- **E2E:** `npm run test:e2e` — Playwright (`test/e2e/tests/`). Shared login: `test/e2e/helpers/auth.ts` (`signInAsTestUser`) matches the seeded user from `test/e2e/scripts/start-server.sh`. Covers **`/health`**, **`/login`** form (**Email address**, **Password**, **Sign in**), sign-in (`signInAsTestUser`), dashboard shell (tokens, **period** links **30 days / 12 months** → **`?period=30d|12m`**, **sidebar `#app-sidebar-nav`** → Transactions / History / Categories / Dashboard, **`.sidebar-fab`** (“Add transaction” → `/transactions`), **global search** `role="search"`, **topbar search** → `/history` with `q`, mobile **Escape** does not collapse drawer while **`details.app-user-menu`** is open), **user menu → Settings** link, notifications link + inbox, mobile sidebar (toggle, **Escape**, **backdrop** dismiss, **#app-sidebar-close**), **footer** `navigation` “Legal and source” (MIT + GitHub), history (**GET** search `q=`; **sort** `<select>` → **`sort=`**; **Clear filters** → **`/history`** with empty query; **kind** tablist **All / Income / Expenses** — preserves other query params and sets **`kind=`**), route smoke (categories, new transaction + **kind** Income/Expense via **`.kind-option`** labels — inputs are visually hidden, settings), **categories modal** (open/close, **Escape**, **backdrop** click outside **`.cat-modal-panel`**, mobile **Escape** with drawer + `dialog` open — sidebar stays open; **showModal** in test harness when backdrop blocks UI), **settings add-member dialog** (open/cancel, **Escape**, **backdrop** click outside card, mobile **Escape** with drawer + `dialog` open — sidebar stays open), **`time.js-local-time` hydration** after saving a transaction (→ `/history`), **logout** (user menu → `/login`), **auth gate** (anonymous `/` → `/login`), **static assets** (`/static/css/app.css`, `/static/js/app.js` non-trivial bodies), **`modulepreload`** for `app.js` on login and dashboard.  
