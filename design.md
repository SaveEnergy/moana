...
- **Mobile shell Escape:** `shellSidebar.ts` uses a **capture-phase** document listener and calls **`keyEventInvolvesOpenDialog`** (`dialogKeyboard.ts`) so Escape is not used to close the drawer while an open **`<dialog>`** is in the event path.
...
- **Unit:** `npm run test:unit` — Vitest, config **merged into `frontend/vite.config.ts`** (`test` block). Covers `frontend/src/lib/*`, **`bootApp()` wiring** (`boot.test.ts`), timezone cookie segment + **`parseMoanaTimezoneCookie`**, **`eventPathIncludesOpenDialog`** (`dialogKeyboard.test.ts`), `formatLocalTimeLabel` / `applyLocalTimeElements`, category custom hex sanitization (`categoryColor.test.ts`), **`onMediaQueryChange`** (`shellBreakpoints.test.ts`).  
