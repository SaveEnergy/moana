...
- **Unit:** `npm run test:unit` — Vitest, config **merged into `frontend/vite.config.ts`** (`test` block). Covers `frontend/src/lib/*`, **`bootApp()`** / **`BOOT_APP_INITIALIZERS`** wiring and initializer order (`boot.test.ts`), timezone cookie segment + **`parseMoanaTimezoneCookie`**, **`shouldDeferMobileShellEscape`** and related helpers (`dialogKeyboard.test.ts`; **`APP_USER_MENU_OPEN_SELECTOR`** in **`domSelectors.ts`>), `formatLocalTimeLabel`, **`createLocalTimeLabelMemo`**, `applyLocalTimeElements` (`localTime.test.ts`), category custom hex sanitization (`categoryColor.test.ts`), **`setRadioCheckedByValue`** (`radioMap.test.ts`), **`onMediaQueryChange`** (`shellBreakpoints.test.ts`), **`clickEventTargetElement`** (`clickTarget.test.ts`), **`shouldCloseNativeDialogFromClick`** (`dialogDismiss.test.ts`), **`shouldCloseMobileSidebarFromShellClick`** (`mobileShellDismiss.test.ts`; **`APP_SIDEBAR_CLOSE_SELECTOR`** + shell ids, **`domSelectors.ts`]), **`wireHistorySortAutoSubmit`** (`historyControls.test.ts`; **`HISTORY_SORT_ELEMENT_ID`**, **`domSelectors.ts`]), **`readDataConfirmMessage`**, **`attachConfirmBeforeSubmit`** (`confirmSubmitForms.test.ts`).  
