...
| **JS** | `frontend/src/main.ts` loads CSS then calls **`bootApp()`** from `frontend/src/boot.ts`, which iterates **`BOOT_APP_INITIALIZERS`** (**timezone cookie → local times → shell → settings dialog → category modal → history sort auto-submit → `data-confirm` submit guards**; order is regression-tested). Each module **no-ops** when its DOM is absent (e.g. no `#app-shell` on login-only pages, no category modal on dashboard-only loads). **Mobile shell width** (`1023px`) is defined once as `MOBILE_SHELL_MEDIA_QUERY` in `frontend/src/lib/shellBreakpoints.ts`; `onMediaQueryChange` wraps `addEventListener` with a legacy `addListener` fallback for `shellSidebar.ts`. **Mobile drawer dismiss:** **`shouldCloseMobileSidebarFromShellClick`** (`mobileShellDismiss.ts`), wired from **`shellSidebar.ts`** — one bubbling **`click`** on **`#app-shell`** when the resolved target is **`#app-sidebar-backdrop`** or **`closest('#app-sidebar-close')`** (**`clickEventTargetElement`**). **Escape vs layered UI:** **`shouldDeferMobileShellEscape`** (`dialogKeyboard.ts`) combines **`keyEventInvolvesOpenDialog`** (`composedPath()` + open **`dialog`**) and **`isAppUserMenuDetailsOpen`** (`details.app-user-menu[open]`) so the mobile shell does not steal Escape from modals or the topbar account **`<details>`** (the drawer can cover the bar while `[open]` stays true). **Category modal** default preview strip uses shared `CATEGORY_MODAL_DEFAULT_PREVIEW_BG` in `categoryColor.ts`; color/icon preset radios are indexed **once** in a `Map` by `value` for fast edit opens (**`setRadioCheckedByValue`** in **`radioMap.ts`** applies dataset-driven values with an **auto** fallback instead of chained **`get()`** + assertions); **form-level delegation** handles custom color `input` and color/icon **`change`** on `#cat-modal-form` instead of per-control listeners; **Edit** actions use **event delegation** on **`.cat-list-section`** with **`clickEventTargetElement`** then **`closest('.cat-modal-open-edit')`** (same shared target helper as dialog dismiss); **native `<dialog>` dismiss** for settings + categories uses **`attachNativeDialogDismiss`** in **`dialogDismiss.ts`** (backdrop + **`closest()`** on close/cancel controls; **`clickEventTargetElement`** handles **Text** nodes inside labels/×). **Local times:** `formatLocalTimeLabel` uses one shared `Intl.DateTimeFormat`; **`createLocalTimeLabelMemo`** keeps one pass over `time.js-local-time[datetime]`, memoizing successful labels in a `Map` and recording invalid ISO strings in a `Set` so bad attributes are not re-parsed on every duplicate row. |
...
- **Top bar:** `.app-topbar` — mobile menu control, global search (`.app-search`, GET `/history`), notifications link (`.app-topbar-icon-btn`; **`aria-current="page"`** when **Active** is notifications), `<details class="app-user-menu">` for account.  
...
- **Confirm-before POST:** `form[data-confirm="…"]` — `window.confirm` on **`submit`** (`confirmSubmitForms.ts`; **`readDataConfirmMessage`** trims the attribute and skips blank / whitespace-only values); avoid inline handlers
...
- **History:** Day groupings, `.history-card` rows, search/sort/date range (**from** / **to** + **Apply dates** submits **`GET /history`** with `from=` & `to=`); **`#history-sort`** **`change`** → **`form.requestSubmit()`** (`historyControls.ts`, no inline handler)  
- **Categories:** Modal create/edit; color presets from `internal/category/colors.go`; icon grid; preview; **dismiss** — **`attachNativeDialogDismiss`** on **`#cat-modal`** (backdrop + **`#cat-modal-close`**)  
- **Settings:** Float-field forms, add-member (**`attachNativeDialogDismiss`**: backdrop, close, Cancel), member list with role badges  
...
- **Unit:** `npm run test:unit` — Vitest, config **merged into `frontend/vite.config.ts`** (`test` block). Covers `frontend/src/lib/*`, **`bootApp()`** / **`BOOT_APP_INITIALIZERS`** wiring and initializer order (`boot.test.ts`), timezone cookie segment + **`parseMoanaTimezoneCookie`**, **`shouldDeferMobileShellEscape`** and related helpers (`dialogKeyboard.test.ts`), `formatLocalTimeLabel`, **`createLocalTimeLabelMemo`**, `applyLocalTimeElements` (`localTime.test.ts`), category custom hex sanitization (`categoryColor.test.ts`), **`setRadioCheckedByValue`** (`radioMap.test.ts`), **`onMediaQueryChange`** (`shellBreakpoints.test.ts`), **`clickEventTargetElement`** (`clickTarget.test.ts`), **`shouldCloseNativeDialogFromClick`** (`dialogDismiss.test.ts`), **`shouldCloseMobileSidebarFromShellClick`** (`mobileShellDismiss.test.ts`), **`wireHistorySortAutoSubmit`** (`historyControls.test.ts`), **`readDataConfirmMessage`**, **`attachConfirmBeforeSubmit`** (`confirmSubmitForms.test.ts`).  
- **E2E:** `npm run test:e2e` — Playwright (`test/e2e/tests/`). Shared login: `test/e2e/helpers/auth.ts` (`signInAsTestUser`) matches the seeded user from `test/e2e/scripts/start-server.sh`. `test/e2e/helpers/dates.ts` (`todayLocalISODate`) supplies local calendar **date** values for transaction form tests. Covers **`/health`**, **`/login`** form (**Email address**, **Password**, **Sign in**, **Remember me**, **OAuth** stub group with disabled Google / GitHub), sign-in (`signInAsTestUser`), dashboard shell (**`--primary`** token, **Statistics and outflow period** group, **Total Income** KPI, **Money movement** section, **period** links **30 days / 12 months** → **`?period=30d|12m`**, **sidebar `#app-sidebar-nav`** → Transactions / History / Categories / Dashboard, **`.sidebar-fab`** (“Add transaction” → `/transactions`), **global search** `role="search"`, **topbar search** → `/history` with `q`, mobile **Escape** does not collapse drawer while **`details.app-user-menu`** is open), **user menu → Settings** link, notifications page (empty state copy, topbar **`aria-current="page"`** on the bell link), mobile sidebar (toggle, **Escape**, **backdrop** dismiss, **#app-sidebar-close**), **footer** `navigation` “Legal and source” (MIT + GitHub), history (**GET** search `q=`; **sort** `<select>` → **`sort=`**; **Apply dates** → **`from=`** / **`to=`**; **Clear filters** → **`/history`** with empty query; **kind** tablist **All / Income / Expenses** — preserves other query params and sets **`kind=`**), route smoke (categories, new transaction + **kind** Income/Expense via **`.kind-option`** labels — inputs are visually hidden; **edit entry** from **history** → **`/transactions/{id}/edit`** (**Edit entry** + **Save changes** → **`/history`**); settings), **categories modal** (open/close, **Escape**, **backdrop** click outside **`.cat-modal-panel`**, mobile **Escape** with drawer + `dialog` open — sidebar stays open; **showModal** in test harness when backdrop blocks UI; **delete** runs **`window.confirm`** via **`data-confirm`** — dismiss keeps row, accept removes), **settings add-member dialog** (open/cancel, **Escape**, **backdrop** click outside card, mobile **Escape** with drawer + `dialog` open — sidebar stays open), **`time.js-local-time` hydration** after saving a transaction (→ `/history`), **logout** (user menu → `/login`), **auth gate** (anonymous `/` → `/login`), **static assets** (`/static/css/app.css`, `/static/js/app.js` non-trivial bodies), **`modulepreload`** for `app.js` on login and dashboard.  
