...
- **History:** Day groupings, `.history-card` rows, search/sort/date range (**from** / **to** + **Apply dates** submits **`GET /history`** with `from=` & `to=`); kind filter tabs — **`.history-seg-active`** on the tab matching **`kind=`** (`all` / `expense` / `income`); **`#history-sort`** **`change`** → **`form.requestSubmit()`** (`historyControls.ts`, no inline handler)  
