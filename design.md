...
| **JS** | `frontend/src/main.ts` loads CSS then calls **`bootApp()`** from `frontend/src/boot.ts`, which runs the same `lib/*` initializers in order. Each module **no-ops** when its DOM is absent (e.g. no `#app-shell` on login-only pages, no category modal on dashboard-only loads). **Mobile shell width** (`1023px`) is defined once as `MOBILE_SHELL_MEDIA_QUERY` in `frontend/src/lib/shellBreakpoints.ts`; `onMediaQueryChange` wraps `addEventListener` with a legacy `addListener` fallback for `shellSidebar.ts`. **Escape vs layered UI:** `keyEventInvolvesOpenDialog` uses `composedPath()` for open **`dialog`** elements; **`isAppUserMenuDetailsOpen`** uses `details.app-user-menu[open]` so the mobile shell does not steal Escape when the topbar account **`<details>`** is open (the drawer can cover the bar while `[open]` stays true). **Category modal** default preview strip uses shared `CATEGORY_MODAL_DEFAULT_PREVIEW_BG` in `categoryColor.ts`; color/icon preset radios are indexed **once** in a `Map` by `value` for fast edit opens; **Edit** actions use **event delegation** on **`.cat-list-section`** (`closest('.cat-modal-open-edit')`) so the list does not attach one listener per row and shell clicks stay outside the handler. **Local times:** `formatLocalTimeLabel` uses one shared `Intl.DateTimeFormat`; `applyLocalTimeElements` memoizes labels per distinct `datetime` value within a single pass so repeated timestamps do not re-format. |
...
- **Mobile shell Escape:** `shellSidebar.ts` uses a **capture-phase** document listener and defers when **`keyEventInvolvesOpenDialog`** (open **`dialog` in path**) or **`isAppUserMenuDetailsOpen`** (topbar account menu) is true.
...
- **Unit:** `npm run test:unit` — Vitest, config **merged into `frontend/vite.config.ts`** (`test` block). Covers `frontend/src/lib/*`, **`bootApp()` wiring** (`boot.test.ts`), timezone cookie segment + **`parseMoanaTimezoneCookie`**, **`eventPathIncludesOpenDialog`** / **`eventPathIncludesOpenDetails`** (`dialogKeyboard.test.ts`), `formatLocalTimeLabel` / `applyLocalTimeElements`, category custom hex sanitization (`categoryColor.test.ts`), **`onMediaQueryChange`** (`shellBreakpoints.test.ts`).  
