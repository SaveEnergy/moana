...
- **History:** Day groupings, `.history-card` rows, search/sort/date filters; **`#history-sort`** **`change`** → **`form.requestSubmit()`** (`historyControls.ts`, no inline handler)  
