...
- **Confirm-before POST:** `form[data-confirm="…"]` (**`FORM_DATA_CONFIRM_SELECTOR`**, **`DATA_CONFIRM_ATTRIBUTE`**, **`domSelectors.ts`**) — `window.confirm` on **`submit`** (`confirmSubmitForms.ts`; **`readDataConfirmMessage`** trims the attribute and skips blank / whitespace-only values); avoid inline handlers
...
- **History:** Day groupings, `.history-card` rows, search/sort/date range (**from** / **to** + **Apply dates** submits **`GET /history`** with `from=` & `to=`); kind filter tabs — **`.history-seg-active`** on the tab matching **`kind=`** (`all` / `expense` / `income`); **`#history-sort`** (id **`HISTORY_SORT_ELEMENT_ID`** from **`domSelectors.ts`**) **`change`** → **`form.requestSubmit()`** (`historyControls.ts`, no inline handler)  
...
- **Settings:** Float-field forms, readonly **Email** field `#settings-email` bound to the signed-in user, add-member (**`domSelectors.ts`**: **`SETTINGS_ADD_MEMBER_*`**; **`attachNativeDialogDismiss`**: backdrop, close, Cancel), member list with role badges  
...
- **Unit:** `npm run test:unit` — Vitest, config **merged into `frontend/vite.config.ts`** (`test` block). Covers `frontend/src/lib/*`, **`bootApp()`** / **`BOOT_APP_INITIALIZERS`** wiring and initializer order (`boot.test.ts`, `bootInitializers.test.ts` — **`BOOT_APP_INITIALIZERS`** function names / order), timezone cookie segment + **`parseMoanaTimezoneCookie`**, **`setBrowserTimezoneCookie`**, **`shouldDeferMobileShellEscape`** and related helpers (`dialogKeyboard.test.ts`; **`APP_USER_MENU_OPEN_SELECTOR`** in **`domSelectors.ts`>), `formatLocalTimeLabel`, **`createLocalTimeLabelMemo`**, `applyLocalTimeElements` (`localTime.test.ts`; **`LOCAL_TIME_ELEMENTS_SELECTOR`**, **`TIME_DATETIME_ATTRIBUTE`**, **`domSelectors.ts`]), category modal preview background + custom hex (`categoryColor.test.ts`); **`readCategoryEditRowDataset`** (`categoryModalDataset.test.ts`); category modal radios, **data-confirm**, and local time selectors (`domSelectors.test.ts`); **`CATEGORY_MODAL_*`** (**`categoryModal.ts`**, **`domSelectors.ts`]), **`setRadioCheckedByValue`** (`radioMap.test.ts`), **`MOBILE_SHELL_MAX_WIDTH_PX`** / **`MOBILE_SHELL_MEDIA_QUERY`**, **`onMediaQueryChange`** (`shellBreakpoints.test.ts`), **`initShellSidebar`** (`shellSidebar.test.ts`; **`MOBILE_SHELL_MEDIA_QUERY`**, **`domSelectors.ts`), **`clickEventTargetElement`** (`clickTarget.test.ts`), **`shouldCloseNativeDialogFromClick`**, **`attachNativeDialogDismiss`** (`dialogDismiss.test.ts`), **`shouldCloseMobileSidebarFromShellClick`** (`mobileShellDismiss.test.ts`; **`APP_SIDEBAR_CLOSE_SELECTOR`** + shell ids, **`domSelectors.ts`]), **`wireHistorySortAutoSubmit`**, **`initHistoryControls`** (`historyControls.test.ts`; **`HISTORY_SORT_ELEMENT_ID`**, **`domSelectors.ts`]), **`initSettingsMemberDialog`** (`settingsMemberDialog.test.ts`; **`SETTINGS_ADD_MEMBER_*`**), **`readDataConfirmMessage`**, **`attachConfirmBeforeSubmit`**, **`initConfirmSubmitForms`** (`confirmSubmitForms.test.ts`; **`FORM_DATA_CONFIRM_SELECTOR`**, **`DATA_CONFIRM_ATTRIBUTE`**, **`domSelectors.ts`]).  
