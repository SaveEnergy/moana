...
- **Unit:** `npm run test:unit` — Vitest, config **merged into `frontend/vite.config.ts`** (`test` block). Covers `frontend/src/lib/*`, **`bootApp()`** / **`BOOT_APP_INITIALIZERS`** wiring and initializer order (`boot.test.ts`), timezone cookie segment + **`parseMoanaTimezoneCookie`**, **`shouldDeferMobileShellEscape`** and related helpers (`dialogKeyboard.test.ts`; **`APP_USER_MENU_OPEN_SELECTOR`** in **`domSelectors.ts`>), `formatLocalTimeLabel`, **`createLocalTimeLabelMemo`**, `applyLocalTimeElements` (`localTime.test.ts`; **`LOCAL_TIME_ELEMENTS_SELECTOR`**, **`TIME_DATETIME_ATTRIBUTE`**, **`domSelectors.ts`]), category custom hex sanitization (`categoryColor.test.ts`); category modal radio selectors (`domSelectors.test.ts`); **`CATEGORY_MODAL_*`** (**`categoryModal.ts`**, **`domSelectors.ts`]), **`setRadioCheckedByValue`** (`radioMap.test.ts`), **`onMediaQueryChange`** (`shellBreakpoints.test.ts`), **`clickEventTargetElement`** (`clickTarget.test.ts`), **`shouldCloseNativeDialogFromClick`** (`dialogDismiss.test.ts`), **`shouldCloseMobileSidebarFromShellClick`** (`mobileShellDismiss.test.ts`; **`APP_SIDEBAR_CLOSE_SELECTOR`** + shell ids, **`domSelectors.ts`]), **`wireHistorySortAutoSubmit`**, **`initHistoryControls`** (`historyControls.test.ts`; **`HISTORY_SORT_ELEMENT_ID`**, **`domSelectors.ts`]), **`initSettingsMemberDialog`** (`settingsMemberDialog.test.ts`; **`SETTINGS_ADD_MEMBER_*`**), **`readDataConfirmMessage`**, **`attachConfirmBeforeSubmit`**, **`initConfirmSubmitForms`** (`confirmSubmitForms.test.ts`; **`FORM_DATA_CONFIRM_SELECTOR`**, **`DATA_CONFIRM_ATTRIBUTE`**, **`domSelectors.ts`]).  
