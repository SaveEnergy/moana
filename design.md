...
- **Categories:** Modal create/edit; color presets from `internal/category/colors.go`; icon grid; preview; **dismiss** — delegated **`click`** on **`#cat-modal`** (backdrop + **`closest('#cat-modal-close')`**)  
...
- **Unit:** `npm run test:unit` — Vitest, config **merged into `frontend/vite.config.ts`** (`test` block). Covers `frontend/src/lib/*`, **`bootApp()` wiring and initializer order** (`boot.test.ts`), timezone cookie segment + **`parseMoanaTimezoneCookie`**, **`shouldDeferMobileShellEscape`** and related helpers (`dialogKeyboard.test.ts`), `formatLocalTimeLabel` / `applyLocalTimeElements`, category custom hex sanitization (`categoryColor.test.ts`), **`onMediaQueryChange`** (`shellBreakpoints.test.ts`).  
