...
- **Confirm-before POST:** `form[data-confirm="…"]` (**`FORM_DATA_CONFIRM_SELECTOR`**, **`DATA_CONFIRM_ATTRIBUTE`**, **`domSelectors.ts`**) — `window.confirm` on **`submit`** (`confirmSubmitForms.ts`; **`initConfirmSubmitForms`** uses **`resolveBootContentQueryRoot`** (**`contentRoot.ts`**) like local time so **`findDataConfirmForms`** runs under **`main.app-main`** when present; **`readDataConfirmMessage`** trims the attribute and skips blank / whitespace-only values, then **`attachConfirmBeforeSubmit`** wires **`submit`** — idempotent per form via **`WeakSet`**); avoid inline handlers  
