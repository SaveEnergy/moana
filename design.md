...
- **Unit:** `npm run test:unit` — Vitest, config **merged into `frontend/vite.config.ts`** (`test` block). Covers `frontend/src/lib/*`, **`bootApp()`** / **`BOOT_APP_INITIALIZERS`** wiring and initializer order (`boot.test.ts`), timezone cookie segment + **`parseMoanaTimezoneCookie`**, **`shouldDeferMobileShellEscape`** and related helpers (`dialogKeyboard.test.ts`), `formatLocalTimeLabel` / `applyLocalTimeElements`, category custom hex sanitization (`categoryColor.test.ts`), **`setRadioCheckedByValue`** (`radioMap.test.ts`), **`onMediaQueryChange`** (`shellBreakpoints.test.ts`), **`clickEventTargetElement`** (`clickTarget.test.ts`), **`shouldCloseNativeDialogFromClick`** (`dialogDismiss.test.ts`), **`shouldCloseMobileSidebarFromShellClick`** (`mobileShellDismiss.test.ts`), **`wireHistorySortAutoSubmit`** (`historyControls.test.ts`), **`attachConfirmBeforeSubmit`** (`confirmSubmitForms.test.ts`).  
