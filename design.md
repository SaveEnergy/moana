...
- **Bootstrap idempotency:** **`WeakSet`** guards in **`shellSidebar`** (`initShellSidebar`), **`confirmSubmitForms`**, **`historyControls`**, **`dialogDismiss`** (`attachNativeDialogDismiss` records the dialog **after** `addEventListener`), **`settingsMemberDialog`**, and **`categoryModal`** prevent stacking listeners when **`bootApp()`** runs more than once (tests or future hot reload)
