...
| `internal/server` | `router.go` (middleware: `RequestLogging` → `WithSecurityHeaders` → `WithPostAuthRateLimit` + `WithRequestTimeout` + `WithMaxRequestBodyBytesAdaptive` → mux); `securityheaders.go`, `ratelimit.go`, `http_server.go` (`NewHTTPServer` — connection timeouts, 10s read-header cap); `static.go` + `health.go` + `static_health.go` (`/static/`, `/health`); `logging.go`; `graceful.go` (SIGINT/SIGTERM); delegates app routes to `handlers.RegisterRoutes`. `router_test.go` includes `TestServeMux_GET_rootExactMatch` (stdlib `GET /{$}` vs prefix `GET /`). |
...
| `internal/historyview` | History page: `page_types.go` (`PageData`, `Nav`, `DayGroup`), `page.go` (`BuildPage`), `rows.go` (`n=` page size + quick local date-range links), `limit.go` (fetch `n+1` to detect truncation, `trimHistoryRows`), `query_parse.go` (`ParseHistoryURL`), `nav.go` (`BuildNav`), `groups.go` (`GroupByDay`) — no `net/http`. |
