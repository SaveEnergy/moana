...
- Files: `integration_server_test.go` (login, health, static `/static/`, dashboard period, smoke pages, history filters), `integration_login_test.go`, `integration_logout_test.go`, `integration_transactions_test.go`, `integration_categories_test.go` (create/delete/duplicate, update rename, invalid update id), `integration_settings_test.go` (profile, password change + mismatch + wrong current, household name, member add/remove, invalid remove id, member cannot remove owner).
