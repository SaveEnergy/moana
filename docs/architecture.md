...
- Files: `integration_server_test.go` (login, health GET+HEAD, static `/static/`, unauthenticated `/` → `/login`, dashboard period, smoke pages, history filters), `integration_login_test.go`, `integration_logout_test.go` (session clear + unauthenticated POST), `integration_transactions_test.go` (create/edit validation, edit `next` query + `safepath.Internal` open-redirect guard, edit 404), `integration_categories_test.go` (create/delete/duplicate, delete removes row, update rename, invalid update id), `integration_settings_test.go` (GET `?err=`, profile, password change + mismatch + wrong current, household name + empty name, member add + duplicate + member denied, remove flows, owner blocked from leave when others remain, sole owner leave, invalid remove id, member cannot remove owner).
